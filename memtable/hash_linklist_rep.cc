...
HashLinkListRep::HashLinkListRep(
  ...
  char* mem = allocator_->AllocateAligned(sizeof(Pointer) * bucket_size, "HashLinkList",
...
KeyHandle HashLinkListRep::Allocate(const size_t len, char** buf) {
  char* mem = allocator_->AllocateAligned(sizeof(Node) + len, "HashLinkList");
  Node* x = new (mem) Node();
  *buf = x->key;
...
void HashLinkListRep::Insert(KeyHandle handle) {
  ...
  if (first_next_pointer->load(std::memory_order_relaxed) == nullptr) {
    ...
    auto* mem = allocator_->AllocateAligned(sizeof(BucketHeader), "HashLinkList");
  ...
  if (header->GetNumEntries() == threshold_use_skiplist_) {
    ...
    auto mem = allocator_->AllocateAligned(sizeof(SkipListBucketHeader),"HashLinkList");
...
MemTableRep::Iterator* HashLinkListRep::GetIterator(Arena* alloc_arena) {
  ...
  if (alloc_arena == nullptr) {
    ...
    auto mem = alloc_arena->AllocateAligned(sizeof(FullListIterator), "HashLinkListIterator");
...
MemTableRep::Iterator* HashLinkListRep::GetDynamicPrefixIterator(
  ...
  if (alloc_arena == nullptr) {
    ...
    auto mem = alloc_arena->AllocateAligned(sizeof(DynamicIterator), "HashLinkListDynamicIterator");
