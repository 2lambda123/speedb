...
void MemoryAllocatedBytesTest(size_t huge_page_size) {
  ...
  req_sz = 12 * 1024;
  for (int i = 0; i < N; i++) {
    arena.Allocate(req_sz, "arena_test");
  }
  ...
  arena.Allocate(Arena::kInlineSize - 1, "arena_test");
...
static void ApproximateMemoryUsageTest(size_t huge_page_size) {
  ...
  arena.AllocateAligned(kAlignUnit, "arena_test");
  EXPECT_TRUE(arena.IsInInlineBlock());
  arena.AllocateAligned(Arena::kInlineSize / 2 - (2 * kAlignUnit), "arena_test");
  EXPECT_TRUE(arena.IsInInlineBlock());
  arena.AllocateAligned(Arena::kInlineSize / 2, "arena_test");
  EXPECT_TRUE(arena.IsInInlineBlock());
  ...
  // first allocation
  arena.AllocateAligned(kEntrySize, "arena_test");
...
static void SimpleTest(size_t huge_page_size) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    char* r;
    if (rnd.OneIn(10)) {
      r = arena.AllocateAligned(s, "arena_test");
    } else {
      r = arena.Allocate(s, "arena_test");
    }
...
TEST_F(ArenaTest, UnmappedAllocation) {
  ...
  for (int i = 0;; ++i) {
    char* p = arena.Allocate(kBlockSize, "arena_test");
